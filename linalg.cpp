...
double percent=0;
    for(int i=0;i<n;i++)
    {
		...
		percent+=100./n;
		if(((int)percent)%10==0) std::cout << percent << "%\n";
    }
    return res;
}
...
std::vector<std::vector<double> > blockMatMul(const std::vector<std::vector<double> > &A, const std::vector<std::vector<double> > &B)
{
	int n=A.size();
	std::vector<std::vector<double> > res;
	res.resize(n);
	...
	//double percent=0;
	int b=50;
	for(int i0=1;i0<n;i0+=b)
	{
		for(int j0=1;j0<n;j0+=b)
		{
			for(int k0=1;k0<n;k0+=b)
			{
				int ii=i0+b-1<n ? i0+b-1 : n;
				for(int i=i0-1;i<ii;i++)
				{
					int jj=j0+b-1<n ? j0+b-1 : n;
					for(int j=j0-1;j<jj;j++)
					{
						int kk=k0+b-1<n ?k0+b-1 : n;
						
						for(int k=k0-1;k<kk;k++)
						{
							res[i][j]+=A[i][k]*B[k][j];
						}
					}
